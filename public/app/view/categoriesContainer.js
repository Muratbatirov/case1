/*
 * File: app/view/categoriesContainer.js
 *
 * This file was generated by Sencha Architect version 4.2.9.
 * http://www.sencha.com/products/architect/
 *
 * This file requires use of the Ext JS 7.3.x Modern library, under independent license.
 * License of Sencha Architect does not include license for Ext JS 7.3.x Modern. For more
 * details see http://www.sencha.com/license or contact license@sencha.com.
 *
 * This file will be auto-generated each and everytime you save your project.
 *
 * Do NOT hand edit this file.
 */

Ext.define('Case.view.categoriesContainer', {
    extend: 'Ext.Container',
    alias: 'widget.categoriescontainer',

    requires: [
        'Case.view.categoriesContainerViewModel',
        'Case.view.categoriesContainerViewController',
        'Case.view.editcategorytree',
        'Ext.grid.Tree',
        'Ext.Toolbar',
        'Ext.Button'
    ],

    controller: 'categoriescontainer',
    viewModel: {
        type: 'categoriescontainer'
    },
    layout: 'hbox',

    items: [
        {
            xtype: 'container',
            flex: 1,
            reference: 'innercontainer',
            items: [
                {
                    xtype: 'editcategorytree'
                }
            ]
        },
        {
            xtype: 'toolbar',
            docked: 'top',
            items: [
                {
                    xtype: 'button',
                    cls: 'createButton',
                    text: 'Добавить',
                    listeners: {
                        tap: 'onDobavitCategory'
                    }
                }
            ]
        }
    ]

});